...
class BookRepository : BookFacade {
    private fun resultRowToBook(row: ResultRow) =
        Book(
            id = row[Books.id],
    ...
    suspend fun allBooksForFacade(): List<Book> =
        DatabaseFactory.dbQuery {
            Books.selectAll().map { resultRowToBook(it) }
        }

    override suspend fun allBooks(userId: Int): List<Book> =
        DatabaseFactory.dbQuery {
    ...
    override suspend fun findBookByIsbn10orIsbn13(
        isbn10: String,
        isbn13: String,
    ): Book? =
        DatabaseFactory.dbQuery {
    ...
    override suspend fun findBookByIsbn10orIsbn13AndUserId(
        isbn10: String,
        isbn13: String,
        userId: Int,
    ): Book? =
    ...
    override suspend fun deleteBook(id: Int): Boolean =
        DatabaseFactory.dbQuery {
            Books.deleteWhere { Books.id eq id } > 0
        }

    override suspend fun findBookById(id: List<Int>): List<Book> =
        DatabaseFactory.dbQuery {
